...
int main(int argc , char *argv[]){
    ...
    int rank, size;
     int arr_size = 10000;
    FILE *file = fopen("non_blocking_send_recv_3b.csv", "a");
    if(argc == 2){
    ...
    if(rank == 0){
        ...
        MPI_Isend(arr, arr_size, MPI_INT, 1, 0, MPI_COMM_WORLD , &sender);
        MPI_Wait(&sender, MPI_STATUS_IGNORE);
    }
    if(rank == 1 ){
        MPI_Irecv(arr, arr_size, MPI_INT, 0, 0, MPI_COMM_WORLD, &receiver);
        MPI_Wait(&receiver, MPI_STATUS_IGNORE);
    }
     MPI_Barrier(MPI_COMM_WORLD);
    if(rank == 0 ){
